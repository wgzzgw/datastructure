...
public class RBTree {
     ...
     */
    private void leftRotate(RBNode<Integer> x){
        //1. 将y的左子节点赋给x的右子节点，并将x赋给y左子节点的父节点(y左子节点非空时)
     ...
     */
    private void rightRotate(RBNode<Integer> y){
        //1. 将y的左子节点赋给x的右子节点，并将x赋给y左子节点的父节点(y左子节点非空时)
    ...
    public void insert(Integer key){
        RBNode<Integer> node = new RBNode<Integer>(true, key, null, null, null);
        if(node != null){
            insert(node);
        }
    }
    ...
    private void insertFixUp(RBNode<Integer> node){
        RBNode<Integer> parent,gparent;//定义父节点和祖父节点

        //需要修正的条件：父节点存在，且父节点的颜色是红色
        while((parent=node.parent) != null && parent.color){
            gparent = parent.parent;//获得祖父节点,必定有祖父

            //若父节点是祖父节点的左子节点，下面的else相反
            if(parent == gparent.left){
                RBNode<Integer> uncle = gparent.right;//获得叔叔节点
                ...
                if(uncle!=null&&!uncle.color&&node == parent.right){
                    leftRotate(parent);//从父节点出左旋
                    RBNode<Integer> tmp = parent;//然后将父节点和自己调换一下，为下面右旋做准备
                    parent = node;
                    node = tmp;
                }
    ...
    public void setRed(RBNode<Integer> node){
        node.color=true;
    }
    //中序遍历
    public void infixOrder(RBNode<Integer> current){
        if(current != null){
